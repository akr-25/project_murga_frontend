...
function ReqHistory() {
  ...
  useEffect(() => {
    ...
    async function fetchLogs(batch_id){
        try{
            let res = await fetch("http://localhost:3001/api/feedConsumptionLog/fetchAll/?batch_id=" + batch_id, {
                method: "GET",
            }); 
            ...
            //console.log("YES");
            console.log(res.data.feedconsumptionlogs); 

            // console.log(tmpArray[0].BalanceLogs[0].net_balance_type1);
            // console.log(tmpArray[0].BalanceLogs[0].net_balance_type2);

            if(res.message === "success"){
                const fetchedData = res.data.feedconsumptionlogs.map((currentRow, index) => MapData(currentRow, index));
                allData = fetchedData;
  ...
  return (
    <div>
      <h1 className="pageTitle">Feed Logs</h1>
      ...
      <div className="tableDiv">
        <Table striped bordered hover responsive>
          ...
          <TBody rowFor="FL" tableData={currentRows}></TBody>
        </Table>
