    ...
    if(res.message === "success"){
       console.log("res status 200 recieved");
        if(newOrderStatus === "Pending For Completion"){
            ...
            // generating pdf
            const orderYear = item.createdAt.substring(0, 4);
            const orderMonth = item.createdAt.substring(5, 7);
            const orderDate = item.createdAt.substring(8, 10);
            
            const statusYear = item.createdAt.substring(0, 4);
            const statusMonth = item.createdAt.substring(5, 7);
            const statusDate = item.createdAt.substring(8, 10);
            
            const obj = {
              sNo: 0,
              orderNo: item.request_id,
              custName: item.User.first_name + " " + item.User.last_name,
              amt: "Rs. " + item.selling_price_per_unit,
